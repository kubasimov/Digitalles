    ...
    public static class GetDefinitions
    {
        ...
        public static List<string> GetList(ref string textToRecognize)
        {
            ...
            for (var i = 0; i < listmatch.Count; i++)
            {
                ...
                var stop = i == listmatch.Count-1 ? textToRecognize.Length-start : listmatch[i + 1].Index -start;

                listDefinitions.Add(textToRecognize.Substring(start,stop));
                
            }

            textToRecognize = listDefinitions.Aggregate(textToRecognize, (current, definition) => current.Replace(definition, ""));
